...
class MoERouter(nn.Module):
    ...
    def set_metrics_enabled(self, enabled: bool) -> None:
        if self._metrics_enabled == enabled:
            return
    ...
    def reset_metrics(self):
        if self._batch_size_per_expert is not None:
            self._batch_size_per_expert.x.zero_()
        if self._score_bias_batch_size_per_expert is not None:
            self._score_bias_batch_size_per_expert.x.zero_()
        if self._load_balancing_loss is not None:
            self._load_balancing_loss.x.zero_()
        if self._z_loss is not None:
            self._z_loss.x.zero_()
