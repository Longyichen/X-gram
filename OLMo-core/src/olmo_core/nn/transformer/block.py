...
from typing import Iterable, TYPE_CHECKING, Dict, Optional, Tuple, Union, cast
...
from ..embedding_injection.mort import apply_mort_sparse_injection
...
def _apply_retoken_modulation(
    ff_out: Tensor,
    embeddings: Optional[Iterable[nn.Embedding]],
    scalers: Optional[Iterable[torch.nn.Parameter]],
    input_ids: Optional[Tensor],
    *,
    epsilon: float = 1e-6,
) -> Tensor:
    """
    Apply ReToken modulation to the feed-forward output when available.
    """
    if embeddings is None or scalers is None or input_ids is None:
        return ff_out

    for embedding_layer, scaler_param in zip(embeddings, scalers):
        mod = embedding_layer(input_ids)
        mod_fp32 = mod.to(torch.float32)
        scaler = scaler_param.to(torch.float32).view(1, 1, -1)

        denom = torch.linalg.vector_norm(mod_fp32, dim=-1, keepdim=True) + epsilon
        normalized = mod_fp32 / denom

        gate = 1.0 + scaler * normalized
        ff_out = ff_out * gate.to(dtype=ff_out.dtype, device=ff_out.device)
...
class TransformerBlock(TransformerBlockBase):
    ...
    def __init__(
        ...
        *,
        ...
        self.mort_sparse_weight_generator: Optional[nn.Module] = None
        self.mort_sparse_scaler: Optional[torch.nn.Parameter] = None
        self.mort_sparse_top_k: Optional[int] = None
        self.mort_sparse_aux_loss_weight: float = 1e-4
        self.mort_sparse_aux_loss: Optional[torch.Tensor] = None
    ...
    def forward(
        ...
        *,
        loss_div_factor: Optional[Union[torch.Tensor, float]] = None,
        _injection_h_embeddings: Optional[Iterable[nn.Embedding]] = None,
        _injection_h_gates: Optional[Iterable[nn.Module]] = None,
        _retoken_embeddings: Optional[Iterable[nn.Embedding]] = None,
        _retoken_scalers: Optional[Iterable[torch.nn.Parameter]] = None,
        input_ids: Optional[torch.Tensor] = None,
        _mort_sparse_weight_generator: Optional[nn.Module] = None,
        _mort_sparse_scaler: Optional[torch.nn.Parameter] = None,
        _mort_sparse_top_k: Optional[int] = None,
        **kwargs,
    ) -> torch.Tensor:
        del loss_div_factor

        injection_version = kwargs.get("_injection_version", "None")
        mort_sparse_injection = None
        mort_sparse_aux_loss: Optional[torch.Tensor] = None
        self.mort_sparse_aux_loss = None
        mort_sparse_weight_generator = _mort_sparse_weight_generator or self.mort_sparse_weight_generator
        mort_sparse_scaler = _mort_sparse_scaler or self.mort_sparse_scaler
        mort_sparse_top_k = _mort_sparse_top_k if _mort_sparse_top_k is not None else self.mort_sparse_top_k
        ...
        if qkv_injection_present and (_kwargs_targets := kwargs.get("_injection_targets", [])) and any(
            t in {"q", "k", "v"} for t in _kwargs_targets
        ):
            kwargs["input_ids"] = input_ids
        elif kwargs.get("_injection_o_delta") is not None:
        ...
        attn_out = self.attention(attn_norm, **kwargs)
        h = x + self.dropout(attn_out)
        # Reuse the post-attention RMSNorm state for both injection and FFN.
        h_norm = self.feed_forward_norm(h)
        if injection_version == "Mort":
            depth_scale = 1.0 / math.sqrt(2 * self.n_layers)
            top_k = mort_sparse_top_k if mort_sparse_top_k is not None else 1
            mort_sparse_injection, mort_sparse_aux_loss = apply_mort_sparse_injection(
                h_norm,
                _injection_h_embeddings,
                mort_sparse_weight_generator,
                mort_sparse_scaler,
                input_ids,
                top_k=top_k,
                load_balancing_loss_weight=self.mort_sparse_aux_loss_weight,
                depth_scale=depth_scale,
            )
        ff_out = self.feed_forward(h_norm)
        ff_out = _apply_retoken_modulation(
            ff_out,
            _retoken_embeddings,
            _retoken_scalers,
            input_ids,
        ...
        if mort_sparse_injection is not None:
            output = output + mort_sparse_injection
        if mort_sparse_aux_loss is not None:
            # Block keeps the latest load-balancing auxiliary loss so trainers can aggregate it.

            self.mort_sparse_aux_loss = mort_sparse_aux_loss
        return output
    ...
    def apply_tp(
        ...
        if self.mort_sparse_weight_generator is not None:
            parallelize_module(
                self.mort_sparse_weight_generator,
                device_mesh=tp_mesh,
...
class ReorderedNormTransformerBlock(TransformerBlock):
    ...
    def forward(
        ...
        *,
        loss_div_factor: Optional[Union[torch.Tensor, float]] = None,
        _injection_h_embeddings: Optional[nn.ModuleList] = None,
        _injection_h_gates: Optional[Union[nn.ParameterList, nn.ModuleList]] = None,
        _retoken_embeddings: Optional[Iterable[nn.Embedding]] = None,
        _retoken_scalers: Optional[Iterable[torch.nn.Parameter]] = None,
        input_ids: Optional[torch.Tensor] = None,
        **kwargs,
        ...
        if any(
            ...
            kwargs["input_ids"] = input_ids
            # print(
            #     f"[DEBUG] block.forward kwargs keys before attention: {list(kwargs.keys())}",
            #     flush=True,
            # )
        ...
        ff_out = self.feed_forward(self.feed_forward_norm(h))
        ff_out = _apply_retoken_modulation(
            ff_out,
            _retoken_embeddings,
            _retoken_scalers,
