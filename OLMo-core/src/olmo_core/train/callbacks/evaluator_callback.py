...
class EvaluatorCallback(Callback):
    ...
    def post_train(self):
        if self.cancel_after_first_eval or not self.evaluators or self.step <= 0:
            return
        ...
        if ran_eval and hasattr(self.trainer, "_log_metrics"):
            try:
                self.trainer._log_metrics()
            except Exception:
                log.warning("Failed to flush metrics after final eval", exc_info=True)
    ...
    def _perform_eval(self):
        ...
        for names, bs, t in sorted_evaluators:
            for metric_name in names:
                eval_speeds.append(f"    {metric_name}: {t:.1f} sec ({bs} batches)")
...
class DownstreamEvaluator(Evaluator):
    metric_type_to_label = {
        "f1_v1": "F1 score",
        "acc_v1": "accuracy",
        "acc_len_norm_v1": "length-normalized accuracy",
        ...
        "f1_v2": "F1 score v2",
        "acc_v2": "accuracy v2",
        "acc_len_norm_v2": "length-normalized accuracy v2",
