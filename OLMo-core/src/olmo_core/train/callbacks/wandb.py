...
class WandBCallback(Callback):
    ...
    def post_train(self):
        if self.enabled and get_rank() == 0 and self.run is not None:
            ...
            # Wait for all trainer async tasks to finish. Note that
            # trainer._shutdown() waits for all thread-pool tasks, but
            # post_train() runs before _shutdown(), so we wait manually here.
            if hasattr(self.trainer, "_multi_thread_pool") and self.trainer._multi_thread_pool:
                # Wait for all pending bookkeeping tasks to finish.
                for op_name, ops in list(self.trainer._bookkeeping_queue.items()):
                    for op_id, future in list(ops.items()):
                        future.result()  # Block until completion.
            log.info("wandb run left open for background flush; process exit will finalize it")

    def on_error(self, exc: BaseException):
        del exc
        if self.enabled and get_rank() == 0 and self.run is not None:
            log.warning("Training failed; wandb run will be finalized automatically on exit")
