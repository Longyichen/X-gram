...
@dataclass
class TransformerMetricsConfig(Config):
    """
    Controls optional training-time diagnostics that are disabled by default.
    """

    enabled: bool = False
    log_batch_size: bool = False
    log_effective_length: bool = False
    log_loss_per_byte: bool = False
    log_module_grad_norm: bool = False
    log_layer_grad_norm: bool = False
    log_last_activation_norm: bool = False
    log_embedding_norm: bool = False
    log_param_mean: bool = False
    param_mean_keywords: Tuple[str, ...] = (
        "embedding",
        "injection",
        "injection_shortconvs",
        "conv",
        "conv_content",
        "gate",
        "scalar",
    )
    log_moe_metrics: bool = False
    metadata_byte_keys: Tuple[str, ...] = (
        "num_bytes",
        "bytes",
        "byte_count",
        "num_chars",
