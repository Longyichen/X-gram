...
class TransformerPipelineTrainModule(TrainModule):
    ...
    def __init__(
        ...
        self._is_final_stage = self.pp_group_rank == self.pp_final_stage_rank
    ...
    def _initialize_activation_trackers(self) -> None:
        ...
        if not self._metrics_enabled or self.metrics_config is None:
            return
        stage_prefix = f"activation_norm/stage_{self.pp_group_rank:02d}"
        for model_part in self.model_parts:
            if (
                self.metrics_config.log_last_activation_norm
                and getattr(model_part, "lm_head", None) is not None
                and f"{stage_prefix}/last_hidden" not in self._activation_trackers
            ):
                self._activation_trackers[f"{stage_prefix}/last_hidden"] = ActivationTracker(
                    model_part.lm_head, pre_hook=True
                )
            if (
                self.metrics_config.log_embedding_norm
                and getattr(model_part, "embeddings", None) is not None
                and f"{stage_prefix}/embedding" not in self._activation_trackers
            ):
                self._activation_trackers[f"{stage_prefix}/embedding"] = ActivationTracker(
                    model_part.embeddings, pre_hook=False
                )
    ...
    def _log_batch_metrics(
        ...
        *,
        ...
        ce_batch_loss: Optional[torch.Tensor],
    ) -> None:
        if (
            not self._is_final_stage
            or not self._metrics_enabled
            or self.metrics_config is None
            or ce_batch_loss is None
        ):
            return
        cfg = self.metrics_config
        tokens_tensor = torch.tensor(batch_num_tokens_for_loss, dtype=torch.float32)
        ...
        if cfg.log_effective_length and batch_size > 0:
            effective_length = tokens_tensor / float(batch_size)
            self.record_metric(
                "effective length",
                effective_length,
                ReduceType.mean,
        ...
        if cfg.log_loss_per_byte:
            if total_bytes and total_bytes > 0:
                loss_per_byte = ce_batch_loss.detach() * (
                    tokens_tensor.to(ce_batch_loss.device) / float(total_bytes)
                )
                self.record_metric(
                    "loss per byte",
                    loss_per_byte,
    ...
    def _log_gradient_metrics(self) -> None:
        ...
        if self.metrics_config.log_param_mean:
            self._log_all_param_means()
        if not (
            self.metrics_config.log_layer_grad_norm
            or self.metrics_config.log_module_grad_norm
        ):
            return
        for part_idx, model_part in enumerate(self.model_parts):
            ...
            if not norms:
                continue
            prefix = f"stage_{self.pp_group_rank:02d}/part_{part_idx:02d}"
            for key, value in norms.items():
                self.record_metric(
                    f"grad norm/{prefix}/{key}",
                    value,
                    ReduceType.l2_norm,
                    namespace="optim",
                )
            if self.metrics_config.log_module_grad_norm:
                ...
                self._log_grad_norm_summary(
                    summary,
                    stage_prefix=f"Stage {self.pp_group_rank} part {part_idx}",
                )
    ...
    def _log_grad_norm_summary(
        ...
        label = stage_prefix or f"Stage {self.pp_group_rank}"
        summary_str = ", ".join(f"{key}={value:.4f}" for key, value in summary.items())
        if step is not None:
            log.info("%s grad norms at step %d: %s", label, step, summary_str)
        else:
            log.info("%s grad norms: %s", label, summary_str)

    def _log_param_means(
        self, part_idx: int, model_part: Transformer, prefix: str
    ) -> None:
        if (
            not self._metrics_enabled
            or self.metrics_config is None
        ...
        if not keywords:
            return
        means = collect_param_means(model_part, keywords=keywords)
        if not means:
            return
        for key, value in means.items():
            self.record_metric(
                f"param mean/{prefix}/{key}",
                value,
                ReduceType.mean,
                namespace="optim",
            )

    def _log_all_param_means(self) -> None:
        if (
            not self._metrics_enabled
            or self.metrics_config is None
            or not self.metrics_config.log_param_mean
        ...
        for part_idx, model_part in enumerate(self.model_parts):
            prefix = f"stage_{self.pp_group_rank:02d}/part_{part_idx:02d}"
            self._log_param_means(part_idx, model_part, prefix)
    ...
    def train_batch(self, batch: Dict[str, Any], dry_run: bool = False):
        ...
        batch_size = batch.get("input_ids").shape[0]  # type: ignore[union-attr]
        batch_num_tokens_for_loss = (labels != self.label_ignore_index).sum().item()
        total_bytes: Optional[int] = None
        if (
            self._is_final_stage
            and self._metrics_enabled
            and self.metrics_config is not None
