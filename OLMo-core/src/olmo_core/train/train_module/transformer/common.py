...
def parallelize_model(
    ...
    *,
    ...
    ac_disabled = os.environ.get("OLMO_DISABLE_ACTIVATION_CHECKPOINTING", "0").lower() not in {
        "0",
        "false",
        "no",
    }
    if ac_config is not None and not ac_disabled:
        for m in model_parts:
    ...
    elif ac_config is not None and ac_disabled:
        log.info("Activation checkpointing disabled via OLMO_DISABLE_ACTIVATION_CHECKPOINTING")
