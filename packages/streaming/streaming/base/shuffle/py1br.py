...
def _calculate_hash(shard_sizes: NDArray[np.int64],
    ...
    param_str = f"{shard_sizes.tobytes()}_{num_canonical_nodes}_{seed}_{epoch}_{block_size}"
...
def get_shuffle_py1br(shard_sizes: NDArray[np.int64],
    ...
    paramter_hash = _calculate_hash(shard_sizes, num_canonical_nodes, seed, epoch, block_size)
    cache_path = None

    if os.environ.get("STREAMING_CACHE", None) is not None:
        cache_dir = os.environ.get("STREAMING_CACHE")
        assert cache_dir is not None
        cache_path = os.path.join(cache_dir, f"shuffle_py1br_{paramter_hash}.npy")
        if os.path.exists(cache_path):
            print(f"Load the shuffle_py1br_{paramter_hash}.npy from the cache")
    ...
    if cache_path is not None:
        ...
        print(f"Save the shuffle_py1br_{paramter_hash}.npy to the cache")
